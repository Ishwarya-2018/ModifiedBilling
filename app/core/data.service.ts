...
export class DataService {
  ...
  createNewUser(id: number, names: String, amounts: number): UserData {
      let uniqueKey = id + 1;
    return {
  ...
  create100Users(): UserData[] {
    ...
    for (let i = 0; i < this.names.length; i++) {
      users.push(this.createNewUser(i, this.names[i], this.amount[i]));
      this.totalValue = this.totalValue+ users[i].amount;
    }
    return users;
  }
